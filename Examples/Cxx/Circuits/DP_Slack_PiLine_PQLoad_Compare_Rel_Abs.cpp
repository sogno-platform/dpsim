...
int main(int argc, char* argv[]) {
	...
	{
		// ----- DYNAMIC SIMULATION ----- RELATIVE
		Real timeStepDP = timeStep;
		Real finalTimeDP = finalTime+timeStepDP;
		String simNameDP = "DP_Slack_PiLine_PQLoad_DP_Relative";
		...
		extnetDP->setParameters(Complex(Vnom,0), 0.0, ramp, timeStart, duration);
		...
		loggerDP->addAttribute("V_ref", extnetDP->attribute("V_ref"));
	...
	{
		// ----- DYNAMIC SIMULATION ----- ABSOLUTE
		Real timeStepDP = timeStep;
		Real finalTimeDP = finalTime+timeStepDP;
		String simNameDP = "DP_Slack_PiLine_PQLoad_DP_Absolute";
		Logger::setLogDir("logs/" + simNameDP);
		...
		auto extnetDP = DP::Ph1::NetworkInjection::make("Slack", Logger::Level::debug);
		extnetDP->setParameters(Complex(Vnom,0), 0.0, ramp, timeStart, duration, true);
