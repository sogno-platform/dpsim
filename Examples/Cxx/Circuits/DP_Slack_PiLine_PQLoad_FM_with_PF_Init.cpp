...
int main(int argc, char* argv[]) {
	String simName = "DP_Slack_PiLine_PQLoad_FM_with_PF_Init";
	...
	Real modulationFrequency = 1.25;
	Real modulationAmplitude = 1.25;
	bool zigzag = false; 
	...
	if (argc > 1) {
		...
		if (args.name != "dpsim")
			simName = args.name;
		if (args.options.find("zigzag") != args.options.end()) {
			zigzag = args.options["zigzag"];
			simName = "DP_Slack_PiLine_PQLoad_FMzz_with_PF_Init";
		}
	}
	...
	extnetDP->setParameters(Complex(Vnom,0), modulationFrequency, modulationAmplitude, -1.25, zigzag);
	...
	//std::shared_ptr<SwitchEvent> loadStepEvent = CIM::Examples::createEventAddPowerConsumption("n2", 5.0-timeStepDP, 100e3, systemDP, Domain::DP, loggerDP);
