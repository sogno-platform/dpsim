...
#include <cps/SP/SP_Ph1_SynchronGenerator6OrderVBR.h>

using namespace CPS;

SP::Ph1::SynchronGenerator6OrderVBR::SynchronGenerator6OrderVBR
...
SP::Ph1::SynchronGenerator6OrderVBR::SynchronGenerator6OrderVBR
	...
	: SynchronGenerator6OrderVBR(name, name, logLevel) {
}

SimPowerComp<Complex>::Ptr SP::Ph1::SynchronGenerator6OrderVBR::clone(String name) {
	auto copy = SynchronGenerator6OrderVBR::make(name, mLogLevel);
	return copy;
}

void SP::Ph1::SynchronGenerator6OrderVBR::initialize() {
...
void SP::Ph1::SynchronGenerator6OrderVBR::calculateAuxiliarConstants() {
...
void SP::Ph1::SynchronGenerator6OrderVBR::stepInPerUnit() {
	...
	mEh_t(1,0) = mAq_t * mIdq(0,0) + mBq_t * mEdq_t(1,0) + mCq_t;

	// calculate history term behind the subtransient reactance
	mEh_s(0,0) = mAd_s * mIdq(1,0) + mBd_s * mEdq_t(0,0) + mDd_s * mEdq_s(0,0);
	mEh_s(1,0) = mAq_s * mIdq(0,0) + mBq_s * mEdq_t(1,0) + mCq_s + mDq_s * mEdq_s(1,0);
