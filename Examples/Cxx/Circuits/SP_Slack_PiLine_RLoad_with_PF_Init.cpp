...
int main(int argc, char* argv[]) {
	String simName = "SP_Slack_PiLine_RLoad";
	...
	auto loadPF = SP::Ph1::Shunt::make("RLoad", Logger::Level::debug);
	loadPF->setParameters(pLoadNom / std::pow(Vnom, 2), 0.0);
	...
	extnetSP->setParameters(Complex(Vnom,0), 0, -6.25, 0.15, 0.4);
	// extnetSP->setParameters(Complex(Vnom,0), 1.25, 1.25, -1.25);
	
	auto lineSP = SP::Ph1::PiLine::make("RxLine", Logger::Level::debug);
	lineSP->setParameters(lineResistance, lineInductance);

	auto loadSP = SP::Ph1::Load::make("RLoad", Logger::Level::debug);	
	loadSP->setParameters(pLoadNom, 0.0, Vnom);
	
	// Topology
	extnetSP->connect({ n1SP });
	lineSP->connect({ n2SP, n1SP });
	loadSP->connect({ n2SP });	
	...
	loggerSP->addAttribute("isrc", extnetSP->attribute("i_intf"));
	loggerSP->addAttribute("i12", lineSP->attribute("i_intf"));
	loggerSP->addAttribute("iL", loadSP->attribute("i_intf"));
