...
ns = {'cim': 'http://iec.ch/TC57/2009/CIM-schema-cim14#',
...
for terminal in tree.iterfind('cim:Terminal', ns):
    ...
    if condeq_id in pt_ends:
        ...
        pt_id = pt_end.find('cim:PowerTransformerEnd.MemberOf_PowerTransformer', ns).get('{'+ns['rdf']+'}resource')[1:]
        terminal_id = terminal.get('{'+ns['rdf']+'}ID')
        end_term = xml.etree.ElementTree.Element('cim:TransformerEnd.Terminal', {'rdf:resource': '#' + terminal_id})
