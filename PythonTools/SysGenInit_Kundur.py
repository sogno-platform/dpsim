import numpy as np
import matplotlib.pyplot as plt

# Kundur - Exampel 3.1 p. 91
# nominal values
nom_P = 555e6
nom_V = 24e3
nom_f = 60
wElec = 2 * np.pi * nom_f

# stator and rotor parameters in their respective systems
Rs = 0.0031 # armature winding resistance
Ll = 0.4129e-3 # leakage inductance
Lmd = 4.5696e-3 # d-axis mutual inductance
#Lmd = Lmd * 0.835 # saturated value
Lmq = 4.432e-3 # q-axis mutual inductance
#Lmq = Lmq * 0.835 # saturated value
Ld = Lmd + Ll # d-axis self inductance
Lq = Lmq + Ll # q-axis self inductance

Rf = 0.0715 # field winding resistance
Lmfd = 40e-3 # field winding mutual inductance
Lfd = 576.92e-3 # field winding self inductance
Llfd = Lfd - Lmfd # field winding leakage inductance
Rkd = 0.0295 # d-axis damper winding resistance
Llkd = 0.0005 # d-axis damper winding leakage inductance
Rkq1 = 0.0064 # q-axis damper winding 1 resistance
Llkq1 = 0.002 # q-axis damper winding 1 leakage inductance
Rkq2 = 0.0246 # q-axis damper winding 2 resistance
Llkq2 = 0.0003 # q-axis damper winding 2 leakage inductance

J = 28897.6459179918 # inertia
numPolePairs = 2 # number of pole pairs

# scenario values
vtPhPh_RMS_scen = 24e3
P_scen = 555e3
#P_scen = 555e6 * 0.9
Q_scen = 0
#Q_scen = 555e6 * 0.436

# per unit base values
base_v = nom_V * np.sqrt(2) / np.sqrt(3)
base_i = nom_P * np.sqrt(2) / (nom_V * np.sqrt(3))
base_Z = base_v / base_i
base_wElec = 2 * np.pi * nom_f
base_wMech = base_wElec / numPolePairs
base_L = base_Z / base_wElec
base_psi = base_L * base_i

base_ifd = Lmd / Lmfd * base_i
base_vfd = nom_P / base_ifd
base_Zfd = base_vfd / base_ifd
base_Lfd = base_Zfd / base_wElec

# pu values
pu_Rs = Rs / base_Z
pu_Ll = Ll / base_L
pu_Lmd = Lmd / base_L
pu_Lmq = Lmq / base_L
pu_Ld = Ld / base_L
pu_Lq = Lq / base_L

# reactances
Xd = Ld * wElec
Xq = Lq * wElec

# steady state initial values
init_P = P_scen
init_Q = Q_scen
init_S = np.sqrt( pow(init_P,2) + pow(init_Q,2) )
init_vt = vtPhPh_RMS_scen / np.sqrt(3) * np.sqrt(2)
init_it = init_S / (nom_V * np.sqrt(3)) * np.sqrt(2)
init_pf = np.arccos(init_P / init_S)
init_delta = np.arctan( (Xq * init_it * np.cos(init_pf) - Rs * init_it * np.sin(init_pf)) /
	(init_vt + Rs * init_it * np.cos(init_pf) + Xq * init_it * np.sin(init_pf)) ) # load angle
init_delta_deg = init_delta / np.pi * 180

# steady state pu values
pu_init_P = init_P / nom_P
pu_init_Q = init_Q / nom_P
pu_init_S = np.sqrt( pow(pu_init_P,2) + pow(pu_init_Q,2) )
pu_init_vt = vtPhPh_RMS_scen / np.sqrt(3) * np.sqrt(2) / base_v
pu_init_it = pu_init_S / pu_init_vt
pu_init_pf = np.arccos(pu_init_P / pu_init_S)
pu_init_delta = np.arctan( (pu_Lq * pu_init_it * np.cos(pu_init_pf) - pu_Rs * pu_init_it * np.sin(pu_init_pf)) /
	(pu_init_vt + pu_Rs * pu_init_it * np.cos(pu_init_pf) + pu_Lq * pu_init_it * np.sin(pu_init_pf)) ) # load angle
pu_init_delta_deg = pu_init_delta / np.pi * 180

# initial steady state dq0 values
init_vd = init_vt * np.sin(init_delta)
init_vq = init_vt * np.cos(init_delta)
pu_init_vd = pu_init_vt * np.sin(pu_init_delta)
pu_init_vq = pu_init_vt * np.cos(pu_init_delta)

# initialize electrical system
peak_i = init_it
peak_v = init_vt
init_thetaElec = - np.pi / 2
init_ia = peak_i * np.cos(init_thetaElec)
init_ib = peak_i * np.cos(init_thetaElec - 2 * np.pi / 3)
init_ic = peak_i * np.cos(init_thetaElec + 2 * np.pi / 3)
init_va = peak_v * np.cos(init_thetaElec)
init_vb = peak_v * np.cos(init_thetaElec - 2 * np.pi / 3)
init_vc = peak_v * np.cos(init_thetaElec + 2 * np.pi / 3)

sysVabcs = np.array([init_va, init_vb, init_vc]).reshape((3,1))

inductanceMat = np.array([[Lq, 0, 0, Lmq, Lmq, 0, 0],
		[0, Ld, 0, 0, 0, Lmd, Lmd],
		[0, 0, Ll, 0, 0, 0, 0],
		[Lmq, 0, 0, Llkq1 + Lmq, Lmq, 0, 0],
		[Lmq, 0, 0, Lmq, Llkq2 + Lmq, 0, 0],
		[0, Lmd, 0, 0, 0, Llfd + Lmd, Lmd],
		[0, Lmd, 0, 0, 0, Lmd, Llkd + Lmd]])
#print(inductanceMat)

thetaTr = init_thetaElec + init_delta
ParkTransformKrause = np.array([[ 2. / 3. * np.cos(thetaTr), 2. / 3. * np.cos(thetaTr - 2. * np.pi / 3.), 2. / 3. * np.cos(thetaTr + 2. * np.pi / 3.)],
[2 / 3 * np.sin(thetaTr), 2. / 3. * np.sin(thetaTr - 2. * np.pi / 3.), 2. / 3. * np.sin(thetaTr + 2. * np.pi / 3.)],
[1. / 3., 1. / 3., 1. / 3.]])

InvParkTransformKrause = np.array([[ np.cos(thetaTr), np.sin(thetaTr), 1],
[np.cos(thetaTr - 2. * np.pi / 3.), np.sin(thetaTr - 2. * np.pi / 3.), 1],
[np.cos(thetaTr + 2. * np.pi / 3.), np.sin(thetaTr + 2. * np.pi / 3.), 1]])

print(sysVabcs)
vdq0 = np.dot(ParkTransformKrause, sysVabcs)
print(vdq0)

# matlab inital state vector
psi_simulinkCustom = np.array([0.5359, -0.8475, 0.8404, 0.6501, -0.7752, -0.7752])
psi_simulinkPS = np.array([[-0.001761719374406999, 1.0000011128194648, 1.0993119581844952, 1.0000013772220995, -0.0016115792415471568, -0.0016115792415471568]])
