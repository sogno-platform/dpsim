...
aliases: ["/docs/overview/initialization/"]
...
For details on the three-stage composite lifecycle (`createSubComponents`, `initializeParentFromNodesAndTerminals`, `mnaCompInitialize`), see [Subcomponent Handling]({{< ref "/docs/Developer Guide/Writing a Model/subcomponents.md" >}}).
...
{{% alert title="Watch out: do not name a hook initialize(Real)" color="warning" %}}
...
{{% alert title="Watch out: virtual nodes must exist before the solver collects them" color="warning" %}}
A virtual node created for the first time *after* `collectVirtualNodes()` (step 3) never gets a
...
The following diagram shows how the solver and a composite component interact during initialization. For further details see [Subcomponent Handling]({{< ref "/docs/Developer Guide/Writing a Model/subcomponents.md" >}}).
...
{{% alert title="Watch out: overriding this hook makes you responsible for the base call" color="warning" %}}
