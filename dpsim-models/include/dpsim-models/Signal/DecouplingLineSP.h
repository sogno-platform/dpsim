...
namespace CPS {
namespace Signal {
class DecouplingLineSP : public SimSignalComp,
                       public SharedFactory<DecouplingLineSP> {
protected:
  Real mDelay;
  Real mResistance;
  ...
  std::shared_ptr<SP::SimNode> mNode1, mNode2;
  std::shared_ptr<SP::Ph1::Resistor> mRes1, mRes2;
  std::shared_ptr<SP::Ph1::CurrentSource> mSrc1, mSrc2;
...
public:
  typedef std::shared_ptr<DecouplingLineSP> Ptr;
  ...
  DecouplingLineSP(String name, SimNode<Complex>::Ptr node1,
                 SimNode<Complex>::Ptr node2, Real resistance, Real inductance,
                 Real capacitance,
                 Logger::Level logLevel = Logger::Level::info);

  DecouplingLineSP(String name, Logger::Level logLevel = Logger::Level::info);
  ...
  private:
    DecouplingLineSP &mLine;
  };

  class PostStep : public Task {
  public:
    PostStep(DecouplingLineSP &line)
