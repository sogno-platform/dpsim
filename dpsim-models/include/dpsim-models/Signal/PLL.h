...
namespace CPS {
namespace Signal {
class PLL : public SimSignalComp, public SharedFactory<PLL> {
...
public:
  ...
  public:
    PreStep(PLL &pll) : Task(**pll.mName + ".PreStep"), mPLL(pll) {
      mPLL.signalAddPreStepDependencies(
  ...
  public:
    Step(PLL &pll) : Task(**pll.mName + ".Step"), mPLL(pll) {
      mPLL.signalAddStepDependencies(
          mPrevStepDependencies, mAttributeDependencies, mModifiedAttributes);
    }
    void execute(Real time, Int timeStepCount) {
