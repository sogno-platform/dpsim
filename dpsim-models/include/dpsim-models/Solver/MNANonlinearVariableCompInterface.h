...
namespace CPS {
	/// MNA interface to be used by nonlinear elements that require recomputing of the system matrix
	class MNANonlinearVariableCompInterface: virtual public MNAVariableCompInterface {
	public:
		...
		/// Returns value of the component's defining voltage/current equation based on current circuit quantities
		virtual void calculateNonlinearFunctionResult() = 0;
		virtual void iterationUpdate(const Matrix& leftVector) = 0;

	protected:

		MNANonlinearVariableCompInterface(): mNonlinearFunctionStamp(AttributeStatic<Matrix>::make()) { };
	};
}
