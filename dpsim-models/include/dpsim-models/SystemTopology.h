...
public:
  ...
  SystemTopology(Real frequency)
      : mFrequencies(initFrequency(frequency)), mSystemFrequency(frequency),
  ...
  SystemTopology(Real frequency, TopologicalNode::List nodes,
  ...
  SystemTopology(Real frequency, Matrix frequencies,
                 TopologicalNode::List nodes, IdentifiedObject::List components)
      : mFrequencies(frequencies), mSystemFrequency(frequency),
        mSystemOmega(2 * PI * frequency) {
    addNodes(nodes);
    addComponents(components);
    componentsAtNodeList();
  ...
  void connectComponentToNodes(typename SimPowerComp<VarType>::Ptr component,
  ...
  template <typename Type> typename std::shared_ptr<Type> node(UInt index);
  ...
  int checkTopologySubnets(
      std::unordered_map<typename CPS::SimNode<VarType>::Ptr, int> &subnet);
