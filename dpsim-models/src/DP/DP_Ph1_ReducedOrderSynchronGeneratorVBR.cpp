...
void DP::Ph1::ReducedOrderSynchronGeneratorVBR::calculateConductanceMatrix() {
  ...
  resistanceMatrix(0, 1) =
      (mA - mB) / 2.0 + (mA + mB) / 2.0 * cos(2 * DeltaTheta);
  resistanceMatrix(1, 0) =
      -(mA - mB) / 2.0 + (mA + mB) / 2.0 * cos(2 * DeltaTheta);
...
void DP::Ph1::ReducedOrderSynchronGeneratorVBR::mnaCompInitialize(
    Real omega, Real timeStep, Attribute<Matrix>::Ptr leftVector) {

  Base::ReducedOrderSynchronGenerator<Complex>::mnaCompInitialize(
      omega, timeStep, leftVector);
  ...
  if (mModelAsNortonSource) {
    ...
    mVariableSystemMatrixEntries.push_back(std::make_pair<UInt, UInt>(
        matrixNodeIndex(0, 0) + complexOffset, matrixNodeIndex(0, 0)));
    mVariableSystemMatrixEntries.push_back(std::make_pair<UInt, UInt>(
        matrixNodeIndex(0, 0), matrixNodeIndex(0, 0) + complexOffset));
    mVariableSystemMatrixEntries.push_back(
        std::make_pair<UInt, UInt>(matrixNodeIndex(0, 0) + complexOffset,
    ...
    // second off diagonal
    mVariableSystemMatrixEntries.push_back(std::make_pair<UInt, UInt>(
        matrixNodeIndex(0, 0), mVirtualNodes[0]->matrixNodeIndex()));
    mVariableSystemMatrixEntries.push_back(
        std::make_pair<UInt, UInt>(matrixNodeIndex(0, 0) + complexOffset,
                                   mVirtualNodes[0]->matrixNodeIndex()));
    mVariableSystemMatrixEntries.push_back(std::make_pair<UInt, UInt>(
...
void DP::Ph1::ReducedOrderSynchronGeneratorVBR::mnaCompApplySystemMatrixStamp(
    SparseMatrixRow &systemMatrix) {
  if (mModelAsNortonSource) {
    // Stamp conductance matrix
    Math::addToMatrixElement(systemMatrix, matrixNodeIndex(0, 0),
                             matrixNodeIndex(0, 0), mConductanceMatrix);
  } else {
...
void DP::Ph1::ReducedOrderSynchronGeneratorVBR::
    mnaCompApplyRightSideVectorStamp(Matrix &rightVector) {
  if (mModelAsNortonSource) {
    // Determine equivalent Norton current
    mIvbr = Complex(mConductanceMatrix(0, 0) * mEvbr.real() +
                        mConductanceMatrix(0, 1) * mEvbr.imag(),
                    mConductanceMatrix(1, 0) * mEvbr.real() +
                        mConductanceMatrix(1, 1) * mEvbr.imag());
...
void DP::Ph1::ReducedOrderSynchronGeneratorVBR::mnaCompPostStep(
  ...
  **mVdq = mDomainInterface.applyDPToDQTransform((**mIntfVoltage)(0, 0)) /
  ...
  if (mModelAsNortonSource) {
    (**mIntfCurrent)(0, 0) =
        mIvbr -
        Complex(mConductanceMatrix(0, 0) * (**mIntfVoltage)(0, 0).real() +
                    mConductanceMatrix(0, 1) * (**mIntfVoltage)(0, 0).imag(),
                mConductanceMatrix(1, 0) * (**mIntfVoltage)(0, 0).real() +
                    mConductanceMatrix(1, 1) * (**mIntfVoltage)(0, 0).imag());
  } else {
