...
DP::Ph1::SynchronGenerator4OrderTPM::clone(String name) {
  auto copy = SynchronGenerator4OrderTPM::make(name, mLogLevel);

  return copy;
}

void DP::Ph1::SynchronGenerator4OrderTPM::setOperationalParametersPerUnit(
    Real nomPower, Real nomVolt, Real nomFreq, Real H, Real Ld, Real Lq,
    Real L0, Real Ld_t, Real Lq_t, Real Td0_t, Real Tq0_t) {

  Base::ReducedOrderSynchronGenerator<Complex>::setOperationalParametersPerUnit(
      nomPower, nomVolt, nomFreq, H, Ld, Lq, L0, Ld_t, Lq_t, Td0_t, Tq0_t);
  ...
  SPDLOG_LOGGER_INFO(mSLog,
                     ...
                     "Ld_t: {:e}\nLq_t: {:e}\n"
                     "Td0_t: {:e}\nTq0_t: {:e}\n",
                     H, Ld, Lq, L0, Ld_t, Lq_t, Td0_t, Tq0_t);
};

void DP::Ph1::SynchronGenerator4OrderTPM::calculateStateSpaceMatrices() {
  mAStateSpace << -mLq / mTq0_t / mLq_t, 0, 0, -mLd / mTd0_t / mLd_t;
  mBStateSpace << (mLq - mLq_t) / mTq0_t / mLq_t, 0.0, 0.0,
      (mLd - mLd_t) / mTd0_t / mLd_t;
  mCStateSpace << 0, 1 / mTd0_t;
  Math::calculateStateSpaceTrapezoidalMatrices(
      mAStateSpace, mBStateSpace, mCStateSpace, mTimeStep, mAdStateSpace,
      mBdStateSpace, mCdStateSpace);
}
...
void DP::Ph1::SynchronGenerator4OrderTPM::calculateConstantConductanceMatrix() {
  ...
  SPDLOG_LOGGER_INFO(mSLog, "\nR_const [pu]: {}",
                     Logger::matrixToString(resistanceMatrix));
  ...
  SPDLOG_LOGGER_INFO(mSLog, "\nG_const [S]: {}",
                     Logger::matrixToString(mConductanceMatrixConst));
}

void DP::Ph1::SynchronGenerator4OrderTPM::mnaCompApplySystemMatrixStamp(
  ...
  if (mModelAsNortonSource) {
    ...
    Math::addToMatrixElement(systemMatrix, matrixNodeIndex(0, 0),
                             matrixNodeIndex(0, 0), mConductanceMatrixConst);
  } else {
    // Stamp voltage source
    Math::setMatrixElement(systemMatrix, mVirtualNodes[0]->matrixNodeIndex(),
                           mVirtualNodes[1]->matrixNodeIndex(), Complex(-1, 0));
...
void DP::Ph1::SynchronGenerator4OrderTPM::stepInPerUnit() {
  ...
  **mNumIter = 0;
  ...
  SPDLOG_LOGGER_DEBUG(mSLog, "\nR_var [pu] (t={:f}): {:s}", mSimTime,
                      Logger::matrixToString(mResistanceMatrixVarying));

  // predict stator current (linear extrapolation)
  Matrix IdpPrediction = Matrix::Zero(2, 1);
  IdpPrediction(0, 0) =
      2 * (**mIntfCurrent)(0, 0).real() - mIdpTwoPrevStep(0, 0).real();
  IdpPrediction(1, 0) =
      2 * (**mIntfCurrent)(0, 0).imag() - mIdpTwoPrevStep(0, 0).imag();
  ...
  mEh(1, 0) = mAq_t * (**mIdq)(0, 0) + mBq_t * (**mEdq_t)(1, 0) +
              mDq_t * mEf_prev + mDq_t * (**mEf);
  ...
  **mEhMod +=
      -Complex(mBase_Z * (mResistanceMatrixVarying * IdpPrediction)(0, 0), 0);
  **mEhMod +=
      -Complex(0, mBase_Z * (mResistanceMatrixVarying * IdpPrediction)(1, 0));
...
void DP::Ph1::SynchronGenerator4OrderTPM::mnaCompApplyRightSideVectorStamp(
    Matrix &rightVector) {
  if (mModelAsNortonSource) {
    // Determine equivalent Norton current
    mIhMod = Complex(mConductanceMatrixConst(0, 0) * (**mEhMod).real() +
                         mConductanceMatrixConst(0, 1) * (**mEhMod).imag(),
                     mConductanceMatrixConst(1, 0) * (**mEhMod).real() +
                         mConductanceMatrixConst(1, 1) * (**mEhMod).imag());
    // Stamp Norton current
    Math::setVectorElement(rightVector, matrixNodeIndex(0, 0), mIhMod);
  } else {
    // Stamp modified history voltage
    Math::setVectorElement(rightVector, mVirtualNodes[1]->matrixNodeIndex(),
                           **mEhMod);
  }
}
...
void DP::Ph1::SynchronGenerator4OrderTPM::correctorStep() {
  ...
  **mNumIter = **mNumIter + 1;
  ...
  (**mEdq_t) = Math::applyStateSpaceTrapezoidalMatrices(
      mAdStateSpace, mBdStateSpace, mCdStateSpace * **mEf, mEdqtPrevStep,
      **mVdq, mVdqPrevStep);
  ...
  Complex IdpCorrectionComplex =
      mDomainInterface.applyDQToDPTransform(**mIdq) * mBase_I_RMS;
  ...
  **mEhMod +=
      -Complex(mBase_Z * (mResistanceMatrixVarying * IdpCorrection)(0, 0), 0);
  **mEhMod +=
      -Complex(0, mBase_Z * (mResistanceMatrixVarying * IdpCorrection)(1, 0));
...
void DP::Ph1::SynchronGenerator4OrderTPM::updateVoltage(
  ...
  **mVdq = mDomainInterface.applyDPToDQTransform((**mIntfVoltage)(0, 0)) /
           mBase_V_RMS;
}
...
void DP::Ph1::SynchronGenerator4OrderTPM::mnaCompPostStep(
    const Matrix &leftVector) {
  ...
  if (mModelAsNortonSource) {
    (**mIntfCurrent)(0, 0) =
        mIhMod -
        Complex(
            mConductanceMatrixConst(0, 0) * (**mIntfVoltage)(0, 0).real() +
                mConductanceMatrixConst(0, 1) * (**mIntfVoltage)(0, 0).imag(),
            mConductanceMatrixConst(1, 0) * (**mIntfVoltage)(0, 0).real() +
                mConductanceMatrixConst(1, 1) * (**mIntfVoltage)(0, 0).imag());
  } else {
  ...
  **mIdq = mDomainInterface.applyDPToDQTransform((**mIntfCurrent)(0, 0)) /
           mBase_I_RMS;
}
