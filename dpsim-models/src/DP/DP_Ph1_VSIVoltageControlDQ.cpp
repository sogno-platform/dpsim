...
void DP::Ph1::VSIVoltageControlDQ::initializeFromNodesAndTerminals(Real frequency) {
	...
	if(mWithConnectionTransformer)
	{
		...
		**mElecActivePower= **mIrcd * **mVcd + **mIrcq * **mVcq;
        ...
        mVCO->setInitialValues(**mDroopOutput, std::arg(mVirtualNodes[3]->initialSingleVoltage()), std::arg(mVirtualNodes[3]->initialSingleVoltage()));
	}
	else
	{
		// Initialize control subcomponents
		...
		**mElecActivePower= **mIrcd * **mVcd + **mIrcq * **mVcq;
		...
		mDroop->setInitialStateValues(matrixInputInit, matrixStateInit, matrixOutputInit);
		// Input: [OmegaSet] //State: [theta] // Output: [theta]
        mVCO->setInitialValues(**mDroopOutput, std::arg(mVirtualNodes[2]->initialSingleVoltage()), std::arg(mVirtualNodes[2]->initialSingleVoltage()));
	}
