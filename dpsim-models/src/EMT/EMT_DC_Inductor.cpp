...
EMT::DC::Inductor::Inductor(String uid, String name, Logger::Level logLevel)
...
SimPowerComp<Real>::Ptr EMT::DC::Inductor::clone(String name) {
  auto copy = Inductor::make(name, mLogLevel);
  copy->setParameters(**mInductance, **mInitialCurrent);
  return copy;
}

void EMT::DC::Inductor::setParameters(Real inductance, Real initialCurrent) {
...
void EMT::DC::Inductor::initializeFromNodesAndTerminals(Real) {
  if (!mParametersSet)
