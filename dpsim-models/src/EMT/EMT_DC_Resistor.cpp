...
EMT::DC::Resistor::Resistor(String uid, String name, Logger::Level logLevel)
    : TwoTerminalVTypeSSNComp(uid, name, logLevel),
      mResistance(mAttributes->create<Real>("R")) {}

SimPowerComp<Real>::Ptr EMT::DC::Resistor::clone(String name) {
  auto copy = Resistor::make(name, mLogLevel);
  copy->setParameters(**mResistance);
  return copy;
}

void EMT::DC::Resistor::setParameters(Real resistance) {
  ...
  **mResistance = resistance;
  ...
  // Stateless conductance relation: i = v / R.
