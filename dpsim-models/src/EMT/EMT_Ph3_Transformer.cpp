...
void EMT::Ph3::Transformer::mnaCompApplySystemMatrixStamp(SparseMatrixRow& systemMatrix) {
	...
	SPDLOG_LOGGER_DEBUG(mSLog, "\nEquivalent Conductance: {:s}", Logger::matrixToString(mEquivCond));
}
...
void EMT::Ph3::Transformer::mnaCompUpdateVoltage(const Matrix& leftVector) {
	...
	**mIntfVoltage = Matrix::Zero(3,1);
	...
	if (terminalNotGrounded(1)) {
		(**mIntfVoltage)(0, 0) -= Math::realFromVectorElement(leftVector, matrixNodeIndex(1, 0)) * (**mRatio).real();
		(**mIntfVoltage)(1, 0) -= Math::realFromVectorElement(leftVector, matrixNodeIndex(1, 1)) * (**mRatio).real();
		(**mIntfVoltage)(2, 0) -= Math::realFromVectorElement(leftVector, matrixNodeIndex(1, 2)) * (**mRatio).real();
	}
}
