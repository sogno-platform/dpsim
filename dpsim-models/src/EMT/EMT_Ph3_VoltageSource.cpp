...
EMT::Ph3::VoltageSource::VoltageSource(String uid, String name,
                                       Logger::Level logLevel)
    : MNASimPowerComp<Real>(uid, name, true, true, logLevel),
      mVoltageRef(mAttributes->createDynamic<MatrixComp>(
...
void EMT::Ph3::VoltageSource::setParameters(MatrixComp voltageRef,
  ...
  // Complex(1,0) is used as initialPhasor, since magnitude and phase of V_ref are taken into account by updateVoltage
  srcSigFreqRamp->setParameters(Complex(1, 0), freqStart, rocof, timeStart,
                                duration, smoothRamp);
...
void EMT::Ph3::VoltageSource::setParameters(MatrixComp voltageRef,
  ...
  srcSigFm->setParameters(Complex(1, 0), modulationFrequency,
...
void EMT::Ph3::VoltageSource::mnaCompInitialize(
...
void EMT::Ph3::VoltageSource::mnaCompApplySystemMatrixStamp(
...
void EMT::Ph3::VoltageSource::mnaCompApplyRightSideVectorStamp(
    Matrix &rightVector) {
...
void EMT::Ph3::VoltageSource::updateVoltage(Real time) {
  if (mSrcSig != nullptr) {
    mSrcSig->step(time);
    for (int i = 0; i < 3; i++) {
      (**mIntfVoltage)(i, 0) = RMS3PH_TO_PEAK1PH *
                               Math::abs((**mVoltageRef)(i, 0)) *
                               cos(Math::phase(mSrcSig->getSignal()) +
                                   Math::phase((**mVoltageRef)(i, 0)));
    }
  } else {
...
void EMT::Ph3::VoltageSource::mnaCompAddPreStepDependencies(
...
void EMT::Ph3::VoltageSource::mnaCompAddPostStepDependencies(
    AttributeBase::List &prevStepDependencies,
    AttributeBase::List &attributeDependencies,
    AttributeBase::List &modifiedAttributes,
...
void EMT::Ph3::VoltageSource::mnaCompPostStep(
