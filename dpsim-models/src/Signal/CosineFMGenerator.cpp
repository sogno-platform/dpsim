...
void Signal::CosineFMGenerator::setParameters(Complex initialPhasor,
  ...
  **mFreq = frequency;
  ...
  SPDLOG_LOGGER_INFO(mSLog,
                     ...
                     "\nBase Frequency={} [Hz]",
                     Logger::phasorToString(initialPhasor), modulationFrequency,
                     modulationAmplitude, frequency);
}
...
void Signal::CosineFMGenerator::step(Real time) {
  ...
  if (mZigZag) {
    ...
    phase += 2 * mModulationAmplitude *
             (pow(2 * (tmp - floor(tmp)) - 1, 2) - 1) / PI * sign;
    **mFreq = mBaseFrequency +
              mModulationAmplitude * (2 * (tmp - floor(tmp)) - 1) * sign;
  } else {
    phase += mModulationAmplitude / mModulationFrequency *
             sin(2. * PI * mModulationFrequency * time);
    **mFreq = mBaseFrequency +
              mModulationAmplitude * cos(2. * PI * mModulationFrequency * time);
  }
