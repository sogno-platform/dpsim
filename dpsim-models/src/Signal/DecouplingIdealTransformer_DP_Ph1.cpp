...
DecouplingIdealTransformer_DP_Ph1::DecouplingIdealTransformer_DP_Ph1(
...
void DecouplingIdealTransformer_DP_Ph1::setParameters(Real delay,
                                                      Matrix voltageSrcIntfCurr,
...
void DecouplingIdealTransformer_DP_Ph1::createSubComponents() {
  if (mSubCompCreated)
    return;
  ...
  mVoltageSrc->connect({SimNode<Complex>::GND, mVirtualNodes[0]});
  ...
  mCurrentSrc->connect({SimNode<Complex>::GND, mTerminals[1]->node()});
...
void DecouplingIdealTransformer_DP_Ph1::initializeParentFromNodesAndTerminals(
    Real frequency) {
...
void DecouplingIdealTransformer_DP_Ph1::mnaParentInitialize(
...
Complex
DecouplingIdealTransformer_DP_Ph1::interpolate(std::vector<Complex> &data) {
...
Complex
DecouplingIdealTransformer_DP_Ph1::extrapolate(std::vector<Complex> &data) {
  if (mCouplingMethod == CouplingMethod::EXTRAPOLATION_LINEAR) {
...
void DecouplingIdealTransformer_DP_Ph1::postStep() {
...
void DecouplingIdealTransformer_DP_Ph1::mnaParentPreStep(Real time,
                                                         Int timeStepCount) {
...
void DecouplingIdealTransformer_DP_Ph1::mnaParentPostStep(
...
void DecouplingIdealTransformer_DP_Ph1::mnaCompUpdateVoltage(
    const Matrix &leftVector) {
  (**mIntfVoltage)(0, 0) = mVoltageSrc->intfVoltage()(0, 0);
}

void DecouplingIdealTransformer_DP_Ph1::mnaCompUpdateCurrent(
    const Matrix &leftVector) {
  (**mIntfCurrent)(0, 0) = mVoltageSrc->intfCurrent()(0, 0);
}

void DecouplingIdealTransformer_DP_Ph1::mnaParentAddPreStepDependencies(
...
void DecouplingIdealTransformer_DP_Ph1::mnaParentAddPostStepDependencies(
    AttributeBase::List &prevStepDependencies,
    AttributeBase::List &attributeDependencies,
    AttributeBase::List &modifiedAttributes,
