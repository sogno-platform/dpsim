...
void DecouplingIdealTransformer_DP_Ph1::setParameters(
...
void DecouplingIdealTransformer_DP_Ph1::initialize(Real omega, Real timeStep) {
  if (mDelay <= 0) {
    mDelay = 0;
    mBufSize = 1;
    mAlpha = 1;
  ...
  **mSrcCurrentRef = cur1;
  mVoltageSrc->setParameters(mSrcVoltageRef->get());
...
void DecouplingIdealTransformer_DP_Ph1::PreStep::execute(Real time,
                                                         Int timeStepCount) {
...
void DecouplingIdealTransformer_DP_Ph1::PostStep::execute(Real time,
                                                          Int timeStepCount) {
  mITM.postStep();
}

Task::List DecouplingIdealTransformer_DP_Ph1::getTasks() {
  return Task::List(
      {std::make_shared<PreStep>(*this), std::make_shared<PostStep>(*this)});
}

IdentifiedObject::List DecouplingIdealTransformer_DP_Ph1::getComponents() {
  return IdentifiedObject::List({mRes1, mRes2, mVoltageSrc, mCurrentSrc});
}

TopologicalNode::Ptr DecouplingIdealTransformer_DP_Ph1::getVirtualNode() {
  return mVirtualNode;
}
