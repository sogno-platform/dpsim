...
#include <dpsim-models/Signal/DecouplingLineSP.h>

using namespace CPS;
using namespace CPS::SP::Ph1;
using namespace CPS::Signal;

DecouplingLineSP::DecouplingLineSP(String name, SimNode<Complex>::Ptr node1,
...
DecouplingLineSP::DecouplingLineSP(String name, Logger::Level logLevel)
...
void DecouplingLineSP::setParameters(SimNode<Complex>::Ptr node1,
                                   SimNode<Complex>::Ptr node2, Real resistance,
                                   Real inductance, Real capacitance) {
...
void DecouplingLineSP::initialize(Real omega, Real timeStep) {
  if (mDelay < timeStep)
...
Complex DecouplingLineSP::interpolate(std::vector<Complex> &data) {
...
void DecouplingLineSP::step(Real time, Int timeStepCount) {
...
void DecouplingLineSP::PreStep::execute(Real time, Int timeStepCount) {
  mLine.step(time, timeStepCount);
}

void DecouplingLineSP::postStep() {
...
void DecouplingLineSP::PostStep::execute(Real time, Int timeStepCount) {
  mLine.postStep();
}

Task::List DecouplingLineSP::getTasks() {
  return Task::List(
      {std::make_shared<PreStep>(*this), std::make_shared<PostStep>(*this)});
}

IdentifiedObject::List DecouplingLineSP::getLineComponents() {
