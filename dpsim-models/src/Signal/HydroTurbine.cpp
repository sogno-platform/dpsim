...
void HydroTurbine::setParameters(
  ...
  if (auto params = std::dynamic_pointer_cast<Signal::HydroTurbineParameters>(
          parameters)) {
    mParameters = params;
    ...
    std::cout << "Type of parameters class of " << this->name()
              << " has to be HydroTurbineParameters!" << std::endl;
...
void HydroTurbine::initializeFromPowerFlow(Real Pminit) {
  if (Pminit >= 0 && Pminit <= 1) {
    ...
    SPDLOG_LOGGER_INFO(mSLog,
                       "Initial mechanical power of hydro turbine {} in pu "
                       "should be betwenn 0 and 1",
                       this->name());
  std::cout << "Initial mechanical power of hydro turbine " << this->name()
