...
void SteamTurbine::setParameters(
  ...
  if (auto params = std::dynamic_pointer_cast<Signal::SteamTurbineParameters>(
          parameters)) {
    mParameters = params;
    if (mParameters->Fhp + mParameters->Fip + mParameters->Flp != 1.0) {
      ...
      SPDLOG_LOGGER_INFO(mSLog,
                         "The sum of factors Fhp+Fip+Flp of the steam turbine "
                         "{} should be equal to 1!",
                         this->name());
      std::cout << "The sum of factors Fhp+Fip+Flp of the steam turbine "
                << this->name() << " should be equal to 1!" << std::endl;
      throw CPS::Exception();
    }
  } else {
    std::cout << "Type of parameters class of " << this->name()
              << " has to be SteamTurbineParameters!" << std::endl;
...
void SteamTurbine::initializeFromPowerFlow(Real Pminit) {
  if (Pminit >= 0 && Pminit <= 1) {
    ...
    SPDLOG_LOGGER_INFO(mSLog,
                       "Initial mechanical power of steam turbine {} in pu "
                       "should be betwenn 0 and 1",
                       this->name());
    std::cout << "Initial mechanical power of steam turbine " << this->name()
              << " in pu should be betwenn 0 and 1" << std::endl;
    throw CPS::Exception();
  }
}
...
Real SteamTurbine::step(Real Pgv, Real dt) {
  ...
  // Output
  mPm = mPhp * mParameters->Fhp + mPip * mParameters->Fip +
        mPlp * mParameters->Flp;
