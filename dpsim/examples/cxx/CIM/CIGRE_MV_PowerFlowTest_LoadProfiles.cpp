...
int main(int argc, char **argv) {
  ...
  CSVReader csvreader(simName, loadProfilePath, assignList,
                      Logger::Level::info);
  csvreader.assignLoadProfile(system, time_begin, time_step, time_end,
  ...
  for (auto node : system.mNodes) {
    ...
    for (auto comp : system.mComponentsAtNode[node]) {
      if (std::shared_ptr<CPS::SP::Ph1::PiLine> line =
        ...
        logger->logAttribute(
            line->name() + "." + node->name() + ".I",
            line->mCurrent->deriveCoeff<Complex>(isNodeZero ? 0 : 1, 0));
        logger->logAttribute(
            line->name() + "." + node->name() + ".P",
            line->mActivePowerBranch->deriveCoeff<Real>(isNodeZero ? 0 : 1, 0));
        logger->logAttribute(line->name() + "." + node->name() + ".Q",
                             line->mReactivePowerBranch->deriveCoeff<Real>(
                                 isNodeZero ? 0 : 1, 0));
        ...
        lines.push_back(line);
                     ...
                     std::dynamic_pointer_cast<CPS::SP::Ph1::NetworkInjection>(
                         comp)) {
        logger->logAttribute(node->name() + ".Pinj",
                             extnet->mActivePowerInjection);
        logger->logAttribute(node->name() + ".Qinj",
                             extnet->mReactivePowerInjection);
        ...
        logger->logAttribute(
            trafo->name() + "." + node->name() + ".I",
            trafo->mCurrent->deriveCoeff<Complex>(isNodeZero ? 0 : 1, 0));
        logger->logAttribute(trafo->name() + "." + node->name() + ".P",
                             trafo->mActivePowerBranch->deriveCoeff<Real>(
                                 isNodeZero ? 0 : 1, 0));
        logger->logAttribute(trafo->name() + "." + node->name() + ".Q",
                             trafo->mReactivePowerBranch->deriveCoeff<Real>(
                                 isNodeZero ? 0 : 1, 0));
      }
    }
    ...
    if (!lines.empty()) {
      logger->logAttribute(node->name() + ".Pinj",
                           lines.front()->attributeTyped<Real>("p_inj"));
      logger->logAttribute(node->name() + ".Qinj",
                           lines.front()->attributeTyped<Real>("q_inj"));
    } else {
      for (auto comp : system.mComponentsAtNode[node]) {
        if (std::shared_ptr<CPS::SP::Ph1::Transformer> trafo =
                std::dynamic_pointer_cast<CPS::SP::Ph1::Transformer>(comp)) {
          logger->logAttribute(node->name() + ".Pinj",
                               trafo->attributeTyped<Real>("p_inj"));
          logger->logAttribute(node->name() + ".Qinj",
                               trafo->attributeTyped<Real>("q_inj"));
          break;
        }
      }
    }
