...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 1) {
    filenames = Utils::findFiles({"WSCC-09_RX_DI.xml", "WSCC-09_RX_EQ.xml",
  ...
  SystemTopology sys;
  if (SGModel == "4PCM")
    sys = reader2.loadCIM(60, filenames, Domain::DP, PhaseType::Single,
                          CPS::GeneratorType::SG4OrderPCM);
  else if (SGModel == "4TPM")
    sys = reader2.loadCIM(60, filenames, Domain::DP, PhaseType::Single,
                          CPS::GeneratorType::SG4OrderTPM);
  else
  ...
  for (auto comp : sys.mComponents) {
    if (std::dynamic_pointer_cast<
            CPS::Base::ReducedOrderSynchronGenerator<Complex>>(comp)) {
      std::dynamic_pointer_cast<CPS::MNASyncGenInterface>(comp)
          ->setMaxIterations(maxIter);
      std::dynamic_pointer_cast<CPS::MNASyncGenInterface>(comp)->setTolerance(
          tolerance);
    }
  }
  ...
  for (auto comp : sys.mComponents) {
    if (auto genReducedOrder = std::dynamic_pointer_cast<
            CPS::Base::ReducedOrderSynchronGenerator<Complex>>(comp))
  ...
  for (auto comp : sys.mComponents) {
    if (auto genReducedOrder = std::dynamic_pointer_cast<
            CPS::Base::ReducedOrderSynchronGenerator<Complex>>(comp)) {
      logger->logAttribute(genReducedOrder->name() + ".Tm",
                           genReducedOrder->attribute("Tm"));
      logger->logAttribute(genReducedOrder->name() + ".Te",
                           genReducedOrder->attribute("Te"));
      logger->logAttribute(genReducedOrder->name() + ".omega",
                           genReducedOrder->attribute("w_r"));
      logger->logAttribute(genReducedOrder->name() + ".delta",
                           genReducedOrder->attribute("delta"));
      logger->logAttribute(genReducedOrder->name() + ".N",
                           genReducedOrder->attribute("NIterations"));
    }
  }
