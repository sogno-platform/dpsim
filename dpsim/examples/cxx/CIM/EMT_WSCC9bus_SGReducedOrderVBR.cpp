...
int main(int argc, char *argv[]) {
  ...
  SystemTopology sys;
  if (sgType == "3")
    sys = reader2.loadCIM(60, filenames, Domain::EMT, PhaseType::ABC,
                          CPS::GeneratorType::SG3OrderVBR);
  else if (sgType == "4")
    sys = reader2.loadCIM(60, filenames, Domain::EMT, PhaseType::ABC,
                          CPS::GeneratorType::SG4OrderVBR);
  else if (sgType == "6b")
    sys = reader2.loadCIM(60, filenames, Domain::EMT, PhaseType::ABC,
                          CPS::GeneratorType::SG6bOrderVBR);
  else
  ...
  if (withFault) {
    sw->setParameters(CPS::Math::singlePhaseParameterToThreePhase(1e12),
                      CPS::Math::singlePhaseParameterToThreePhase(0.02 * 529));
  ...
  for (auto comp : sys.mComponents) {
    if (auto genReducedOrder = std::dynamic_pointer_cast<
            CPS::Base::ReducedOrderSynchronGenerator<Real>>(comp)) {
