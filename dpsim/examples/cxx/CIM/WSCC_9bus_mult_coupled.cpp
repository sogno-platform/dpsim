...
void multiply_connected(SystemTopology &sys, int copies, Real resistance,
  ...
  for (auto orig_node : nodes) {
    ...
    for (int i = 0; i < nlines; i++) {
      ...
      auto rl_node =
          std::make_shared<DP::SimNode>("N_add_" + std::to_string(counter));
      ...
      cap2->connect(
          {sys.node<DP::SimNode>(nodeNames[i + 1]), DP::SimNode::GND});
      counter += 1;
...
void simulateCoupled(std::list<fs::path> filenames, CommandLineArgs &args,
                     Int copies, Int threads, Int seq = 0) {
  String simName = "WSCC_9bus_coupled_" + std::to_string(copies) + "_" +
