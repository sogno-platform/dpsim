...
static void DP_RL_SourceStep(Real timeStep, Real finalTime, Real voltageStep,
                             Real freqStep, Real stepTime,
                             String extension = "") {
...
static void EMT_RL_SourceStep(Real timeStep, Real finalTime, Real voltageStep,
                              Real freqStep, Real stepTime,
                              String extension = "") {
  ...
  vs->setParameters(1000, voltageStep, 2 * PI * 50, 2 * PI * freqStep, stepTime,
                    0);
...
int main(int argc, char *argv[]) {
  ...
  for (UInt stepIdx = 1; stepIdx <= maxTimeStepIdx; ++stepIdx) {
    ...
    EMT_RL_SourceStep(timeStep, finalTime, 0, 0, stepTime,
                      "_T" + std::to_string(stepIdx));
    DP_RL_SourceStep(timeStep, finalTime, 0, 0, stepTime,
                     "_T" + std::to_string(stepIdx));

    for (UInt voltageStepIdx = 1; voltageStepIdx <= maxVoltageStepIdx;
         ++voltageStepIdx) {
      voltageStep = voltageStepIdx * 100;
      EMT_RL_SourceStep(timeStep, finalTime, voltageStep, 0, stepTime,
                        "_T" + std::to_string(stepIdx) + "_V" +
                            std::to_string(voltageStepIdx));
    ...
    for (UInt freqStepIdx = 1; freqStepIdx <= maxFreqStepIdx; ++freqStepIdx) {
      ...
      EMT_RL_SourceStep(timeStep, finalTime, 0, freqStep, stepTime,
                        "_T" + std::to_string(stepIdx) + "_F" +
                            std::to_string(freqStepIdx));
