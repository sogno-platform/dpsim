...
int main(int argc, char *argv[]) {
  ...
  auto n1DP =
      SimNode<Complex>::make("n1DP", PhaseType::Single, initialVoltage_n1);
  ...
  auto genDP =
      Factory<CPS::Base::ReducedOrderSynchronGenerator<Complex>>::get().create(
  ...
  std::dynamic_pointer_cast<MNASyncGenInterface>(genDP)->setMaxIterations(
      maxIter);
  std::dynamic_pointer_cast<MNASyncGenInterface>(genDP)->setTolerance(
      tolerance);
  ...
  SystemTopology systemDP =
      SystemTopology(gridParams.nomFreq, SystemNodeList{n1DP, n2DP},
  ...
  logger->logAttribute(genDP->name() + ".NIterations",
                       genDP->attribute("NIterations"));
  ...
  // load step event
  std::shared_ptr<SwitchEvent> loadStepEvent =
      Examples::Events::createEventAddPowerConsumption(
          "n1DP", std::round(loadStepEventTime / timeStep) * timeStep,
          gridParams.loadStepActivePower, systemDP, Domain::DP, logger);
