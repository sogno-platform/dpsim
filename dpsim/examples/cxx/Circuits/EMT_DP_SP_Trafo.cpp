...
void simTrafoSP1ph() {
  ...
  SystemTopology sys(50, SystemNodeList{n1, n2},
                     SystemComponentList{vs, trafo, loadRes});
...
void simTrafoDP1ph() {
  ...
  // Parameters
  vs->setParameters(CPS::Math::polar(voltageHVSide, 0));
  trafo->setParameters(voltageHVSide, voltageMVSide, ratio, 0, trafoResistance,
                       trafoInductance);
  loadRes->setParameters(loadResistanceMVSide);
...
void simTrafoEMT3ph() {
  ...
  trafo->setParameters(
      ...
      CPS::Math::singlePhaseParameterToThreePhase(trafoInductance));
  loadRes->setParameters(
      CPS::Math::singlePhaseParameterToThreePhase(loadResistanceMVSide));
