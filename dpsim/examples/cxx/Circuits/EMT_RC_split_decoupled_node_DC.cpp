...
void decoupleNode(SystemTopology &sys, const String &nodeName,
                  ...
                  String method, Eigen::MatrixXd irLine_0, Real i_inf_0) {
...
void doSim(String &name, SystemTopology &sys, Int threads, Real ts,
  ...
  logger->logAttribute(
      "i_rline", sys.component<EMT::Ph1::Resistor>("r_line")->mIntfCurrent, 1,
      1);
  ...
  if (isDecoupled) {
    ...
    logger->logAttribute(
        "i_intf",
        sys.component<Signal::DecouplingIdealTransformer_EMT_Ph1>("itm_n2")
            ->attribute("i_intf"),
        1, 1);
    logger->logAttribute(
        "i_ref",
...
SystemTopology buildTopology(String &name, float r1_r, float c1_c,
                             ...
                             Eigen::MatrixXd n1_v0, Eigen::MatrixXd n2_v0) {
  ...
  auto sys = SystemTopology(50, SystemNodeList{gnd, n1, n2},
...
int main(int argc, char *argv[]) {
  ...
  SPDLOG_LOGGER_INFO(
      DPsim::Logger::get("EMT_RC_split_decoupled_node_DC"),
  ...
  String simNameDecoupled = "EMT_RC_split_decoupled_DC_" + prefix + "_" +
