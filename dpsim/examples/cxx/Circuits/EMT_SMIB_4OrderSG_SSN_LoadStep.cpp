...
int main(int argc, char *argv[]) {
  ...
  String simName = "EMT_SMIB_4OrderSG_SSN_LoadStep";
  Real timeStep = 10e-6;
  Real finalTime = 35;
  ...
  auto genEMT = EMT::Ph3::SynchronGenerator4OrderSSN::make("synGen", logLevel);
  ...
  lineEMT->setParameters(
      Math::singlePhaseParameterToThreePhase(gridParams.lineResistance),
  ...
  // Topology
  genEMT->connect({n1EMT, CPS::EMT::SimNode::GND});
  lineEMT->connect({n1EMT, n2EMT});
  extnetEMT->connect({n2EMT});

  //auto res = CPS::EMT::Ph3::Resistor::make("res",logLevel);
  //res->setParameters(CPS::Math::singlePhaseParameterToThreePhase(0.5));
  //res->connect({n1EMT,CPS::EMT::SimNode::GND});
  ...
  std::shared_ptr<SwitchEvent3Ph> loadStepEvent =
      Examples::Events::createEventAddPowerConsumption3Ph(
          "n1EMT", std::round(loadStepEventTime / timeStep) * timeStep,
          gridParams.loadStepActivePower, systemEMT, Domain::EMT, logger);
  ...
  simEMT.setSolverType(Solver::Type::ITERATIVEMNA);
