...
int main(int argc, char *argv[]) {
  ...
  linePF->setParameters(scenario.lineResistance, scenario.lineInductance,
                        scenario.lineCapacitance, scenario.lineConductance);
  ...
  auto systemPF =
      SystemTopology(scenario.systemNominalFreq, SystemNodeList{n1PF, n2PF},
  ...
  Complex load1_s = 3 * std::pow(scenario.systemNominalVoltage, 2) /
                    (Complex(scenario.loadRes1,
                             scenario.loadInd1 * scenario.systemNominalOmega));
  Real load1ActivePower = load1_s.real();
  Real load1ReactivePower = load1_s.imag();

  Complex load2_s =
      3 * std::pow(scenario.systemNominalVoltage, 2) /
      (Complex(scenario.loadRes2,
               scenario.loadInd2 * scenario.systemNominalOmega -
                   (1 / (scenario.systemNominalOmega * scenario.loadCap2))));
  ...
  load1EMT->setParameters(
      CPS::Math::singlePhasePowerToThreePhase(load1ActivePower),
      CPS::Math::singlePhasePowerToThreePhase(load1ReactivePower),
      scenario.systemNominalVoltage);

  auto load2EMT = EMT::Ph3::RXLoad::make("Load2", Logger::Level::debug);
  load2EMT->setParameters(
      CPS::Math::singlePhasePowerToThreePhase(load2ActivePower),
      CPS::Math::singlePhasePowerToThreePhase(load2ReactivePower),
      scenario.systemNominalVoltage);

  auto pv = EMT::Ph3::VSIVoltageControlDQ::make("pv", "pv",
                                                Logger::Level::debug, false);
  pv->setParameters(scenario.systemNominalOmega, scenario.Vdref, scenario.Vqref,
                    scenario.Pref); //initialise with Pref
  pv->setControllerParameters(
      scenario.KpVoltageCtrl, scenario.KiVoltageCtrl, scenario.KpCurrCtrl,
      scenario.KiCurrCtrl, scenario.systemNominalOmega, scenario.m_p,
      scenario.tau_p, scenario.tau_l); //Initialise with taup taui mp
  ...
  auto load1SwitchEMT =
      CPS::EMT::Ph3::Switch::make("Switch1", Logger::Level::debug);
  load1SwitchEMT->setParameters(
  ...
  auto load2SwitchEMT =
      CPS::EMT::Ph3::Switch::make("Switch2", Logger::Level::debug);
  load2SwitchEMT->setParameters(
      Math::singlePhaseParameterToThreePhase(SwitchOpen),
      Math::singlePhaseParameterToThreePhase(SwitchClosed));
  ...
  auto systemEMT = SystemTopology(
      scenario.systemNominalFreq, SystemNodeList{n1EMT, n2EMT, n3EMT, n4EMT},
      SystemComponentList{pv, lineEMT, load1SwitchEMT, load1EMT, load2SwitchEMT,
                          load2EMT});
  ...
  auto sw1close =
      SwitchEvent3Ph::make(startTimeLoad1Step, load1SwitchEMT, true);
  sim.addEvent(sw1close);
  auto sw2close =
      SwitchEvent3Ph::make(startTimeLoad2Step, load2SwitchEMT, true);
