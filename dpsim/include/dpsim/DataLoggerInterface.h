...
public:
  ...
  virtual void logAttribute(const String &name, CPS::AttributeBase::Ptr attr,
                            UInt rowsMax = 0, UInt colsMax = 0) {
    if (auto attrReal =
            std::dynamic_pointer_cast<CPS::Attribute<Real>>(attr.getPtr())) {
      mAttributes[name] = attrReal;
    } else if (auto attrComp =
                   std::dynamic_pointer_cast<CPS::Attribute<Complex>>(
                       attr.getPtr())) {
        ...
        for (UInt k = 0; k < rowsMax; ++k) {
          for (UInt l = 0; l < colsMax; ++l) {
            mAttributes[name + "_" + std::to_string(k) + "_" +
                        std::to_string(l)] =
                attrMatrix->deriveCoeff<Real>(k, l);
          }
        }
      }
    } else if (auto attrMatrix =
