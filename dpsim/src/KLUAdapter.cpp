...
namespace DPsim
{
	...
	void KLUAdapter::preprocessing(SparseMatrix& systemMatrix, std::vector<std::pair<UInt, UInt>>& listVariableSystemMatrixEntries)
	{
		if(m_symbolic)
		{
		...
		m_symbolic = klu_analyze_partial(n, Ap, Ai, &varying_columns[0], &varying_rows[0], varying_entries, m_partial_method, &m_common);
	...
	void KLUAdapter::partialRefactorize(SparseMatrix& systemMatrix, std::vector<std::pair<UInt, UInt>>& listVariableSystemMatrixEntries)
	{
		if(systemMatrix.nonZeros() != nnz)
		{
