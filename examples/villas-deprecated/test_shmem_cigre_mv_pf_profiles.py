...
def dpsim():
    ...
    csv_files = glob.glob("build/_deps/profile-data-src/CIGRE_MV_NoTap/load_profiles/")[
        0
    ]
    ...
    node_list = {
        k: v for k, v in obj_list.items() if v == "SimNode<std::complex<double> >"
    }
    ...
    for i in range(15):
        ...
        sim.export_attribute(
            sim.get_idobj_attr(objname, "v").derive_coeff(0, 0).derive_mag(), (i * 2)
        )
        sim.export_attribute(
            sim.get_idobj_attr(objname, "v").derive_coeff(0, 0).derive_phase(),
            (i * 2) + 1,
        )
