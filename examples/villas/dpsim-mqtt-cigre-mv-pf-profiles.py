...
intf_mqtt = dpsimpyvillas.InterfaceVillas(
    name="MQTT", config=json.dumps(intf_mqtt_config)
)
...
for i in range(15):
    objname = "N" + str(i)
    intf_mqtt.export_attribute(
        sim.get_idobj_attr(objname, "v").derive_coeff(0, 0).derive_mag(), 2 * i
    )
    intf_mqtt.export_attribute(
        sim.get_idobj_attr(objname, "v").derive_coeff(0, 0).derive_phase(), 2 * i + 1
    )
