...
def villas():
    ...
    nodes = {
        "mqtt": {
            ...
            "in": {
                ...
                "signals": [
                    {"name": "v1", "type": "complex"},
                    {"name": "v2_mag", "type": "float"},
                ],
            },
            "out": {"publish": "/mqtt-dpsim"},
        },
    ...
    paths = [
        {"in": "mqtt", "out": "file1", "hooks": [{"type": "print"}]},
        {"in": "sine", "out": "mqtt", "hooks": [{"type": "print"}]},
    ]
...
def dpsim():
    ...
    intf_config = {
        ...
        "format": "json",
        "host": "mqtt",
        "in": {"subscribe": "/mqtt-dpsim"},
        "out": {"publish": "/dpsim-mqtt"},
    }

    intf = dpsimpyvillas.InterfaceVillas(
        name="dpsim-mqtt", config=json.dumps(intf_config)
    )
...
def test_shmem_import_export():
    ...
    (
        ...
        dpsim()
    )  # intf needs to be extracted from the dpsim-function since the interface object gets deleted otherwise leading to SegFault when starting the simulation
    node = villas()
