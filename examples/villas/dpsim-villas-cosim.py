...
def dpsim0():
    ...
    intf.import_attribute(evs.attr('V_ref'), 0)
...
def dpsim1():
    ...
    sim.add_interface(intf)
    intf.import_attribute(ecs.attr('I_ref'), 0)
    intf.export_attribute(ecs.attr('v_intf').derive_coeff(0,0).derive_scaled(complex(-1,0)), 0)
