...
void EMT::Ph3::SSN::Capacitor::mnaInitialize(Real omega, Real timeStep, Attribute<Matrix>::Ptr leftVector) {
	...
	Matrix Dufour_B_k_hat = timeStep * (2.0 * **mCapacitance).inverse();
	Matrix Dufour_W_k_n = Dufour_B_k_hat;
...
void EMT::Ph3::SSN::Capacitor::mnaApplySystemMatrixStamp(Matrix& systemMatrix) {
	...
	Math::addToMatrixElement(systemMatrix, mVirtualNodes[0]->matrixNodeIndex(PhaseType::B), mVirtualNodes[0]->matrixNodeIndex(PhaseType::B), -Dufour_W_k_n(1, 0));
	Math::addToMatrixElement(systemMatrix, mVirtualNodes[0]->matrixNodeIndex(PhaseType::C), mVirtualNodes[0]->matrixNodeIndex(PhaseType::C), -Dufour_W_k_n(2, 0));
