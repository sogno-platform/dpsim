...
class Reader:
    ...
    def create_dpsim_objects(self):
        ...
        for index, bus in self.mpc_bus_data.iterrows():
            ...
            bus_name = bus_index
            dpsimpy_busses_dict[bus_name] = dpsimpy.sp.SimNode(
            ...
            if bus_type == 1:
                ...
                dpsimpy_comp_dict[load_name] = [
                    dpsimpy.sp.ph1.Load(load_name, dpsimpy.LogLevel.info)
                ]
                dpsimpy_comp_dict[load_name][0].set_parameters(
                    load_p, load_q, load_baseV
                )
                dpsimpy_comp_dict[load_name][0].modify_power_flow_bus_type(
                    dpsimpy.PowerflowBusType.PQ
                )

                # Add connections
                dpsimpy_comp_dict[load_name].append(
                    [dpsimpy_busses_dict[bus_index]]
                )  # [to bus]
            ...
            # Generators
                ...
                dpsimpy_comp_dict[gen_name] = [
                    dpsimpy.sp.ph1.SynchronGenerator(gen_name, dpsimpy.LogLevel.info)
                ]
                dpsimpy_comp_dict[gen_name][0].set_parameters(
                    gen_nom_s, gen_baseV, gen_p, gen_v, dpsimpy.PowerflowBusType.PV
                )
                dpsimpy_comp_dict[gen_name][0].set_base_voltage(gen_baseV)

                # Add connections
                dpsimpy_comp_dict[gen_name].append(
                    [dpsimpy_busses_dict[bus_index]]
                )  # [to bus]
            ...
            # Network injection (slack bus)
                ...
                dpsimpy_comp_dict[extnet_name][0].modify_power_flow_bus_type(
                    dpsimpy.PowerflowBusType.VD
                )

                # Add connections
                dpsimpy_comp_dict[extnet_name].append(
                    [dpsimpy_busses_dict[bus_index]]
                )  # [to bus]
        ...
        for index, branch in self.mpc_branch_data.iterrows():
            ...
            if branch_ratio == 0:
                line = line + 1
                line_name = "line%s_%s-%s" % (
                    line,
                    self.mpc_branch_data.at[index, "fbus"],
                    self.mpc_branch_data.at[index, "tbus"],
                )

                line_fbus = self.mpc_branch_data.at[index, "fbus"]
                line_tbus = self.mpc_branch_data.at[index, "tbus"]

                tmp_fbus = self.mpc_bus_data.loc[
                    self.mpc_bus_data["bus_i"] == line_fbus
                ]
                tmp_tbus = self.mpc_bus_data.loc[
                    self.mpc_bus_data["bus_i"] == line_tbus
                ]
                ...
                line_baseZ = (
                    line_tbus_baseV * line_tbus_baseV / (self.mpc_base_power_MVA * mw_w)
                )
                ...
                dpsimpy_comp_dict[line_name] = [
                    dpsimpy.sp.ph1.PiLine(line_name, dpsimpy.LogLevel.info)
                ]
                dpsimpy_comp_dict[line_name][0].set_parameters(
                    line_r, line_l, line_c, line_g
                )
                dpsimpy_comp_dict[line_name][0].set_base_voltage(line_tbus_baseV)

                # Add connections
                dpsimpy_comp_dict[line_name].append(
                    [
                        dpsimpy_busses_dict[str(line_fbus)],
                        dpsimpy_busses_dict[str(line_tbus)],
                    ]
                )
            ...
            # Transformers
                ...
                transf_s = (
                    self.mpc_branch_data.at[index, "rateA"] * mw_w
                ...
                tmp_fbus = self.mpc_bus_data.loc[
                    self.mpc_bus_data["bus_i"] == transf_fbus
                ]
                tmp_tbus = self.mpc_bus_data.loc[
                    self.mpc_bus_data["bus_i"] == transf_tbus
                ]
                ...
                transf_primary_v = (
                    self.mpc_bus_data.at[tmp_fbus.first_valid_index(), "Vm"]
                    * transf_fbus_baseV
                )
                transf_secondary_v = (
                    self.mpc_bus_data.at[tmp_tbus.first_valid_index(), "Vm"]
                    * transf_tbus_baseV
                )
                ...
                dpsimpy_comp_dict[transf_name] = [
                    dpsimpy.sp.ph1.Transformer(transf_name, dpsimpy.LogLevel.info)
                ]
                ...
                # Add connections
                dpsimpy_comp_dict[transf_name].append(
                    [
                        dpsimpy_busses_dict[str(transf_fbus)],
                        dpsimpy_busses_dict[str(transf_tbus)],
                    ]
                )
